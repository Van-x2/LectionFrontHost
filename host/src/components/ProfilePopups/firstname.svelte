<script>
    ...
    async function submitSetting() {
        const response = await fetch('/api/updateinfo',
            {
            method: 'POST',
            body: JSON.stringify({ prop: 'firstname', value: firstName.toLowerCase(), id: $page.data.session?.user?._id  }),
            headers: {
    ...
    <div class="w-[500px] h-[260px] bg-white border-[3px] border-accent rounded-[20px] p-3">
        ...
        <div class="w-full h-[75%] flex justify-end items-end">
            <div class="w-full h-full flex justify-center items-center">
                <input bind:value={firstName} type="text" class="w-full h-12 bg-gray1 pl-4 rounded-[10px] mx-6 -translate-y-4 text-[20px] appearance-none border-non focus:outline-none" placeholder={(($page.data.session?.user?.firstname)?.split(' ')[0] || '').replace(/\s/g, '').replace(/^(.)(.*)$/, (_, first, rest) => first.toUpperCase() + rest.toLowerCase())}/>
            </div>
